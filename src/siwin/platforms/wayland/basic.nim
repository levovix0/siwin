...
import nativesockets, net, os, posix, sequtils, tables
type
  Id* = distinct uint32
...
proc connect*(name = getEnv("WAYLAND_SOCKET")): Display =
  ...
  if sock == osInvalidSocket: raise WaylandError.newException("Failed to create socket")

  var a = "\1\0" & name
  
  if sock.connect(cast[ptr SockAddr](a[0].addr), uint32 a.len) < 0:
    close sock
    raise WaylandError.newException("Failed to connect to wayland server")
...
method unmarshal*(x: Proxy, op: int, data: seq[uint32]) {.base.} = discard
